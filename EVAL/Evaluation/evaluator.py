...
class SaliencyDataset(Dataset):
    def __init__(self, pred_dir, gt_dir):
        ...
        # 确保预测图和标签图数量一致、顺序对应
        assert len(self.pred_paths) == len(self.gt_paths), "Pred and GT数量不一致！"

    def __len__(self):
        return len(self.pred_paths)

    def __getitem__(self, idx):
        pred = Image.open(self.pred_paths[idx]).convert('L')
...
if __name__ == "__main__":
    # 1. 配置参数
    pred_dir = "/home/zqq/桌面/twj/BBS-Net-master/dataTest/SMT3_CODTest/COD10K"  # 替换为你的预测图文件夹
    gt_dir = "/home/zqq/桌面/twj/datasource/RGBDCOD/test/COD10K/mask"  # 替换为你的真实标签图文件夹
    method_name = "Labnet"  # 你的模型名称（如"MSNet"）
    dataset_name = "COD10K"  # 数据集名称（如"DUTS-TEST"、"ECSSD"）
    output_dir = "./evaluation_results"  # 评估结果保存目录
    cuda = torch.cuda.is_available()  # 是否使用GPU

    # 2. 创建Loader
    dataset = SaliencyDataset(pred_dir, gt_dir)
    loader = DataLoader(dataset, batch_size=1, shuffle=False)  # batch_size必须为1！
